...
function checkKey()
	if keyCnt >= 30 then --long pressed
		...
		keyCnt = 0
		startSmartConfig(1)
	elseif keyCnt>=3 and gpio.read(1) ==1 then --short pressed
		if(sckTcpSrv ~=nil) then sckTcpSrv:close();sckTcpSrv = nil;collectgarbage() end
...
flagRGBLED=1
function toggleRGBLED()	
	flagRGBLED = flagRGBLED+1
	if(flagRGBLED%2==0) then
	rgb_led_open(0,0,0)
	else
...
function gpioInit()
	dofile('micokit_rgb.lc')
	rgb_led_open(0,0,0)
	gpio.mode(1,gpio.INPUT,gpio.PULLUP)--easylinkbutton d1
	keyCnt = 0
end
